...
public class Queens2 {
    ...
    public static Integer[][] survivorSelection(Integer[][] population, Integer[][] children) {
        ...
        for (int i = 0; i < n; i++) {
            combinedPopulation[i]= Arrays.copyOf(population[i], boardSize);
        ...
        for (int index = n; index < lambda; index++) {
            combinedPopulation[index]= new Integer[boardSize];
            System.arraycopy(children[index - n], 0, combinedPopulation[index], 0, boardSize);
        ...
        for (int i = 0; i < n; i++) {
            System.arraycopy(combinedPopulation[i], 0, newPop[i], 0, boardSize);
        }
        // END OF YOUR CODE
        return newPop;
    }
