 ...
 = (int) (Math.random() * total);
    ...
    public static Integer[][] survivorSelection(Integer[][] population, Integer[][] children) {
      ...
      //  System.out.println();
        ...
        Map<Integer,ArrayList<ArrayList<Integer>>> m1=new HashMap<>();
       ...
       for (int i = 0; i < TotalPopulation.length; i++) {
        ...
        if(m1.containsKey(fit)){
            ArrayList<ArrayList<Integer>> v1=m1.get(fit);
            List al = Arrays.asList(TotalPopulation[i]);
            ArrayList<Integer> mm=new ArrayList<>();
            mm.addAll(al);
            v1.add(mm);
            m1.put( Integer.valueOf(fit),v1);
        }else{

            vv.add(fit);
            ArrayList<ArrayList<Integer>> v1=new ArrayList<>();
           // ArrayList al = Arrays.asArrayList(TotalPopulation[i]);
            ArrayList<Integer> al =
            new ArrayList<Integer>();
            Collections.addAll(al, TotalPopulation[i]);
            ArrayList<Integer> mm=new ArrayList<>();
            mm.addAll(al);
  ...
  //     System.out.println(vv);
       Integer m122[]={10,6,4,2,8,5,9,1,3,7};
     //  System.out.println(m1+" "+measureFitness(m122));
Set<Integer> bb = new HashSet<>();
int x1x = 0; // declare and initialize xx
for (Integer integer : vv) {
   // System.out.println(integer);
    if(x1x >= 10) break;
    if(bb.add(integer)) {
     //   System.out.println(integer+"  key");
        ArrayList<ArrayList<Integer>> v2 = m1.get(integer);
        int size = v2.size();
      //  System.out.println("v2 ="+v2+" "+size);
        for (int i = 0; i < size; i++) { // use size() instead of length
            Integer[] arr = new Integer[10];
            // Converting List to Array
     v2.get(i).toArray(arr);
   
     newPop[x1x] = arr; // assign the array to newPop[xx]
            x1x++;
         //   System.out.println(x1x);
            if(x1x >= 10) break; // exit the loop if xx reaches 10
        }
    }
}    
        return newPop;
...
m1.add(Arrays.asList(population[i]));
    ...
    private static int measureFitness(Integer [] m)
    {
